// npm run transfer-50percent:prod
...
async function transferStock() {
    ...
    const percentage = Number(getArgVal('--percentage') || 50);

    console.log('===========================================================');
    console.log('       50% AUTOMATED STOCK TRANSFER SCRIPT                ');
    console.log('===========================================================');
    console.log(`Source Godown (Master)      : ${fromGodownId}`);
    console.log(`Destination Godown (Varachha): ${toGodownId}`);
    console.log(`Transfer Percentage         : ${percentage}%`);
    ...
    if (!fromGodown || !toGodown) {
        ...
        console.log('  node scripts/transfer-50percent-stock.js --from <MASTER_ID> --to <VARACHHA_ID> [--dry-run]\n');
    ...
    try {
        ...
        const transferNo = `TRF-50PCT-${Date.now()}`;
        ...
        const transferRecord = await StockTransfer.create({
            ...
            toGodownId,
            status: 'Received',
            note: `Automated ${percentage}% stock transfer from ${fromGodown.name} to ${toGodown.name}`,
            totalAmount: 0,
            createdBy: 'System (50% Auto Transfer Script)'
        }, { transaction: t });
        ...
        for (const stock of stocksToTransfer) {
            ...
            // Calculate 50% transfer quantity in base units
            const transferBaseUnits = Math.floor(currentBaseUnits * (percentage / 100.0));

            if (transferBaseUnits <= 0) {
                console.log(` - Skipping [${stock.product?.name?.en || 'Product'}] (${stock.variant?.volume || 'Variant'}): Stock is too small (${currentBaseUnits} base units) to transfer ${percentage}%.`);
                continue;
            }
            ...
            console.log(`[TRANSFERRING] Product: "${productNameEn}" | Volume: "${variantVol}"`);
            console.log(`   - Original Source Stock : ${currentBaseUnits} base units`);
            console.log(`   - 50% Transfer Quantity : ${transferBaseUnits} base units (${transferPacks} packs/units)`);
            console.log(`   - Remaining Source Stock: ${newSourceBaseUnits} base units`);

            // 1. Update Source InventoryStock
            await stock.update({ totalBaseUnits: newSourceSourceUnitsOrDefault(newSourceBaseUnits) }, { transaction: t });

            // 2. Find or Create Destination InventoryStock
            ...
            await InventoryTransaction.create({
                ...
                note: `Automated ${percentage}% Transfer Out to ${toGodown.name} (TRF: ${transferNo})`,
                createdBy: 'System (50% Auto Transfer Script)'
            }, { transaction: t });
            ...
            await InventoryTransaction.create({
                ...
                note: `Automated ${percentage}% Transfer In from ${fromGodown.name} (TRF: ${transferNo})`,
                createdBy: 'System (50% Auto Transfer Script)'
            }, { transaction: t });
        ...
        function newSourceSourceUnitsOrDefault(val) {
            return val;
        }
        ...
        if (isDryRun) {
            await t.rollback();
            console.log('\n===========================================================');
            console.log(`[DRY-RUN COMPLETE] Successfully simulated transfer of ${processedCount} variant(s).`);
            ...
            await t.commit();
            console.log('\n===========================================================');
            console.log(`[SUCCESS] Transferred 50% stock for ${processedCount} variant(s).`);
        ...
        await t.rollback();
        console.error('\n[FATAL ERROR] Stock transfer failed. Transaction rolled back completely.');
